...
def generate_warp_mesh(side_len, n_seg, dim):
    ...
    rest_lens = np.linalg.norm(q_np[connectors_np[:, 0]] - q_np[connectors_np[:, 1]], axis=1).astype(np.float32)  # shape (num_edges,)
    ...
    rest_lens_wp = wp.array(rest_lens, dtype=wp.float32, device="cuda")


    print(f"Created {len(q_np)} points and {len(connectors)} connectors")

    return q_wp, connectors_wp, rest_lens_wp
...
def search_dir(q_wp, q_np, connectors_wp, connectors_np, q_next_wp, q_next_np, m_wp, m_np, rest_lengths_wp, rest_lengths_np):
    # 1. Compute projected Hessian (still on CPU)
    projected_hess = IP_hess(q_np, connectors_np, q_next_np, m_np, rest_lengths_np)

    # 2. Compute gradient on GPU, then copy back to CPU
...
q_gpu, conn_gpu, rest_lens_gpu = generate_warp_mesh(side_len, n_seg, dim)
...
"""
q_gpu, conn_gpu, rest_gpu = generate_warp_mesh(side_len, n_seg, dim)
...
# FIX might need to change to rest_length squared?
# initialize simulation
[q_wp, connectors_wp, rest_lens_wp] = generate_warp_mesh(side_len, n_seg, dim)  # node positions and edge node indices
q_np = q_wp.numpy()                # shape (n, 2) if q_wp is wp.vec2
connectors_np = connectors_wp.numpy()  # shape (n_edges, 2)
rest_lens_np = rest_lens_wp.numpy()    # shape (n_edges,)


v = np.array([[0.0, 0.0]] * len(q_wp))             # velocity
m_np = [rho * side_len * side_len / ((dim) * (dim))] * len(q_wp)  # calculate node mass evenly
#m_np = np.array(m_np, dtype=np.float32)  # make sure it's a NumPy array
...
#FIX I am inputting q_wp here, might want q_np here
